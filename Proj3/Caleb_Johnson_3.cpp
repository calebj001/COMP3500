...
void Init(){
    int i, j;
    std::cout<<"m: input the number of processes"<<endl;
    std::cin>>m;
    std::cout<<"n: input the number of types of resources"<<endl;
    std::cin>>n;
    std::cout<<"Max Matrix: input the value of each element left to right, top to bottom"<<endl;
    for(i=0;i<m;i++){
        for(j=0;j<n;j++){
            std::cin>>Max[i][j];
        }
    }
    std::cout<<"Allocation Matrix: input the value of each element left to right, top to bottom"<<endl;
    for(i=0;i<m;i++){
        ...
        for(j=0;j<n;j++){
            ...
            if(Need[i][j]<0){
                std::cout<<"The "<<i+1<<"-th process request "<<j+1<<" resources, which is beyond the maximum resources, please retry"<<endl;
                j--;
                continue;
            }
        }
    }
    std::cout<<"Available Vector: input the value of each element left to right"<<endl;
    for(i=0;i<n;i++){
        std::cin>>Available[i];
    }
}
    ...
    for(i=0;i<m;i++){
        ...
        // #4#END# DO NOT MODIFIE THIS COMMENT LINE!
            // if the system is safe, the resource will be assigned to the processes
            	std::cout<<"The system is safe"<<endl;
            	std::cout<<"The safe sequence is:"<<endl;
            	for(i=0;i<l;i++){
                	std::cout<<p[i];
                	if(i!=l-1){
                    	std::cout<<"-->";
    	...
    	std::cout<<"The system is not safe"<<endl;
    	return false;
	}
}
...
void Bank() // banker's algorithm
{
    ...
    while(1)
    {
        std::cout<<"A process start to request resources. The process number is: "<<endl;
        std::cin>>cusneed;
        std::cout<<"Each type resource requested by this proess"<<endl;
        for(i=0;i<n;i++)
        {
            std::cin>>Request[cusneed][i];
        }
        for(i=0;i<n;i++)
        {
            if(Request[cusneed][i]>Need[cusneed][i])
            {
                std::cout<<"Requested more than needed, please re-try!"<<endl;
                continue;
            }
            if(Request[cusneed][i]>Available[i])
            {
                std::cout<<"Requested more than available, please re-try!"<<endl;
                continue;
            }
        }
        for(i=0;i<n;i++)
        ...
        {
            std::cout<<"Request approved!"<<endl;
        }else{
            std::cout<<"Your request is declined!"<<endl;
            ...
            for(i=0;i<n;i++)
            {
                ...
                Allocation[cusneed][i] = Allocation[cusneed][i--];
        ...
        std::cout<<"Do you want to try again? Yes press y/Y, otherwise press any other key to quit"<<endl;
        std::cin>>again;
        if(again=='y'||again=='Y')
        {
            continue;
