...
// Version: 09/16/2022
// Filename: Caleb_Johnson.cpp
// Additional: If using vim, use command :set tabstop=4 to format correctly
// Sources: No external sources were used in this project except lectures and
//  the cpp reference library for syntax help.
...
void init() // initialize PCB
{
	...
	cout << "ID PRIORITY RUNTIME--->PCB[" << i << "]" << endl;
...
void show() // display info in each loop
{
    ...
    for (i = 0; i < num; i++) 
    {
		...
		char id = pcblist[i].ID;
		int p = pcblist[i].pri;
		int r = pcblist[i].runtime;
		char s = pcblist[i].state;
		cout << id << "\t" << p << "\t" << r << "\t" << s << endl;
